...
@app.route('/zone2', methods=['GET','POST'])
def zone2_page():
    print(f"\nRequest method: {request.method}")
    print(f"\nRequest path: {request.path}")
    if request.method == 'GET':
        # 사용자가 입력한 데이터 가져오기
        month = int(request.form.get('month'))  # 'month' 입력 필드의 값
        day = int(request.form.get('day'))      # 'day' 입력 필드의 값
        hour = int(request.form.get('hour'))    # 'hour' 입력 필드의 값
    ...
    # GET 요청 시 HTML 폼 렌더링
    return render_template('zone2.html',tmap_api_key = tmap_api_key)
