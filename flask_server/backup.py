...
def zone2_page():
    ...
    user_input = user_input_datetime()  # Assume this function returns a tuple or None
    if user_input:
        try:
            month, day, hour = user_input  # Unpack user input
            buttons_visible = True
            month = str(month).zfill(2)
            day = str(day).zfill(2)
            hour = str(hour).zfill(2)
        except ValueError:
            # Log an error if the input is not valid
    # if request.args:  # 사용자가 폼을 제출했을 때
    #     month = request.args.get('month')  # 'month' 입력 필드의 값
    #     day = request.args.get('day')      # 'day' 입력 필드의 값
    #     hour = request.args.get('hour')    # 'hour' 입력 필드의 값
