...
function createCustomIcon(labels, isRevisited) {
    ...
    ctx.closePath();
    ctx.fillStyle = isRevisited ? '#0066ff' : '#3281a8'; // 삼각형 색상
    ctx.fill();
    ...
    // labels가 배열인지 확인하고, 배열이 아닌 경우에도 처리할 수 있도록 방어 코드 추가
    if (Array.isArray(labels)) {
        // 여러 방문 순서 레이블을 나열하여 표시
        labels.forEach((label, index) => {
            ctx.fillStyle = 'white'; // 각 레이블에 대해 색상 설정
            ctx.font = 'bold 12px Arial'; // 각 레이블에 대해 크기 설정
            ctx.textBaseline = 'middle';
            ctx.textAlign = 'center';
            const yOffset = 30 + index * 10; // 레이블 간격 조정 (세로로 나열)
            ctx.fillText(label, 25, yOffset);
        });
    } else {
        // labels가 배열이 아니면 그냥 단일 값 표시
        ctx.fillStyle = 'black'; // 각 레이블에 대해 색상 설정
        ctx.font = 'bold 12px Arial'; // 각 레이블에 대해 크기 설정
        ctx.fillText(labels, 25, 30);
    }
...
function addWaypointMarkers() {
    ...
    waypoints.forEach((waypoint) => {
        ...
        const marker = new Tmapv2.Marker({
            ...
            iconSize: new Tmapv2.Size(70, 80), // 크기를 늘려서 여러 방문 표시
            zIndex: 1000,
            title: waypoint.title,
            label: waypoint.label,
            visible: true, // 마커를 보이도록 설정
            map: map
        });
...
function startRouteAnimation(routeCoordinates) {
    ...
    const animationMarker = new Tmapv2.Marker({
        ...
        icon: "https://lh3.googleusercontent.com/d/free_icon_truck.png", // 애니메이션 마커 아이콘
        map: map
    });
